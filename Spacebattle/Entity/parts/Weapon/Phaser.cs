...
namespace Spacebattle.entity.parts.Weapon
{
    class Phaser:ShipPart,IWeapon
    {
        ...
        public void FireAt(IDamageableEntity target)
        {
            ...
            if ( distance < _range)
                OnGameEngineEvent(this, GameEngineEventArgs.Damage(target, (new DamageSource() { Magnitude = _power -(_power *distance/_range) , Type = DamageType.DRAINING, Origin = Parent.Position })));
            else
